...
if (RESEND_API_KEY) {
  try {
    ...
    console.log(
      `[mailer] RESEND_API_KEY detected (${RESEND_API_KEY.slice(0, 6)}…, ${RESEND_API_KEY.length} chars) — real email sending is ENABLED. Sending as "${MAIL_FROM_NAME} <${MAIL_FROM}>".`
    );
  } catch (e) {
    console.error(
  ...
  console.log('[mailer] No RESEND_API_KEY set in this environment — emails will be SIMULATED (logged only, nothing actually sent).');
}
